...
class RemoveLicense extends AMarketplace
{
    protected function configure(): void
    {
        $this
            ->setName('marketplace:remove-license')
            ->setDescription('Remove license <info>[!] BETA</info>')
            ->setHelp('')
            ->addArgument('license', InputArgument::REQUIRED, 'License to remove');
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $io->title('Remove license <info>[!] BETA</info>');

        \XLite\Core\Marketplace::getInstance()->dropRebuild();

        $result = \XLite\Core\Marketplace::getInstance()->removeLicense($input->getArgument('license'));
