...
class Stripe extends \XLite\Module\XC\Stripe\Model\Payment\AStripe
{
    ...
    public function isConfigured(\XLite\Model\Payment\Method $method)
    {
        return $this->isSettingsConfigured($method)
            ...
            && !empty($this->getEnabledPaymentMethodTypes());
    }

    /**
     * @return array
     */
    public function getEnabledPaymentMethodTypes(): array
    {
        $result = [];
        ...
        if ($this->isAlipayPaymentEnabled()) {
            $result[] = 'alipay';
        }
        if ($this->isGrabPayPaymentEnabled()) {
            $result[] = 'grabpay';
        }
        if ($this->isFPXPaymentEnabled()) {
            $result[] = 'fpx';
        }
    ...
    protected function getCheckoutSessionParams()
    {
        ...
        $params = [
            ...
            'mode'                 => 'payment',
            'payment_method_types' => $this->getEnabledPaymentMethodTypes(),
     ...
     * @return bool
     */
    protected function convertSettingValueToBool($value): bool
    {
        return in_array($value, ['1', 'Y', 'on', true, 'true'], true);
    }

    /**
     * @return bool
     */
    public function isCreditCardPaymentEnabled(): bool
    {
        $method = Main::getStripeMethod();
        return $this->convertSettingValueToBool($method->getSetting('credit_card_method'));
    }

    /**
     * @return bool
     */
    public function isAlipayPaymentEnabled(): bool
    {
        $method = Main::getStripeMethod();
        return $this->convertSettingValueToBool($method->getSetting('alipay_method'));
    }

    /**
     * @return bool
     */
    public function isGrabPayPaymentEnabled(): bool
    {
        $method = Main::getStripeMethod();
        return $this->convertSettingValueToBool($method->getSetting('grab_pay_method'))
            && $this->isGrabPayPaymentAvailable();
    }

    /**
     * @return bool
     */
    public function isGrabPayPaymentAvailable(): bool
    {
        $currencyCode = \XLite::getInstance()->getCurrency()->getCode();
        return in_array($currencyCode, ['SGD', 'MYR'], true);
    }

    /**
     * @return bool
     */
    public function isFPXPaymentEnabled(): bool
    {
        $method = Main::getStripeMethod();
        return $this->convertSettingValueToBool($method->getSetting('fpx_method'))
            && $this->isFPXPaymentAvailable();
    }

    /**
     * @return bool
     */
    public function isFPXPaymentAvailable(): bool
    {
        $currencyCode = \XLite::getInstance()->getCurrency()->getCode();
        return in_array($currencyCode, ['MYR'], true);
    }
}
