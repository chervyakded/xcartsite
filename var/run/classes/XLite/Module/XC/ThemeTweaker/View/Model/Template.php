...
class Template extends \XLite\View\Model\AModel
{
    ...
    protected function setModelProperties(array $data)
    {
        ...
        if (\XLite\Core\Request::getInstance()->template) {
            ...
            if (!$this->getModelObject()->getEnabled()) {
                $localFilePath = $localPath . '.tmp';
            }
        }

        if ($localPath) {
            $localFilePath = $localFilePath ?: $localPath;
            ...
            $fullFilePath = $layout->getFullPathByLocalPath($localFilePath, $interface);

            $savedBody = \Includes\Utils\FileManager::read(LC_DIR_SKINS . $localFilePath);
            if (\Includes\Utils\FileManager::write($fullFilePath, $body)) {
                if ($savedBody) {
                    $this->savedData = [
                        'path' => $fullPath,
                        'body' => $savedBody,
