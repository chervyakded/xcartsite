...
class TaxCore extends \XLite\Base\Singleton
{
    ...
    const PRICE_ADJUSTED = 'PriceAdjusted';
    const FAILED_BACKEND_STATUSES = [\XLite\Model\Payment\BackendTransaction::STATUS_FAILED, \XLite\Model\Payment\BackendTransaction::STATUS_INITIALIZED];
    const OTHER          = 'Other';

    const COLORADO_FEE_TEXT = 'Colorado Fee';
    const COLORADO_FEE_TAX_CODE = 'OF400000';
    const COLORADO_FEE_TAX_NAME = 'coloradoFee';
    const COLORADO_FEE_STATE_CODE = 'CO';
    ...
    public function adjustTransactionRequest(\XLite\Model\Order $order, string $reason, string $reasonDescription = '')
    {
        $messages     = [];
    ...
    public function voidTransactionRequest(\XLite\Model\Order $order, string $reason = self::DOC_VOIDED)
    {
        $dataProvider = new DataProvider\Order($order);
     ...
     * @return boolean
     */
    public function testConnection(array &$messages = array())
    {
    ...
    public function validateAddress($address)
    {
        ...
        $this->log(
            'API request' . PHP_EOL
            . 'URL: ' . $url . PHP_EOL
            . 'Request: ' . var_export($data, true) . PHP_EOL
        ...
        if (!empty($data['messages'])) {
            foreach ($data['messages'] as $message) {
                if (empty($message['details'])) {
                    // System error
                    \XLite\Logger::getInstance()->log($message['summary'], LOG_ERR);
                    $result[] = [
    ...
    protected function assembleAddressValidationMessage(array $message, $address)
    {
        ...
        if (is_array($address) && is_object(current($address))) {
            ...
            switch ($message['refersTo']) {
                ...
                case 'Address.Region':
                    $names = array('state_id', 'custom_state');
                    break;

                case 'Address.Country':
                    $names = array('country_code');
                    break;
    ...
    protected function processTaxes(array $data)
    {
        ...
        if (!empty($data['error'])) {
            ...
            \XLite\Logger::getInstance()->log($errorData['message'], LOG_ERR);
            ...
            foreach ($data['summary'] as $row) {
                if ($row['tax'] > 0) {
                    ...
                    if (isset($row['jurisType'])
                        && strtolower($row['jurisType']) === 'special'
                    ) {
                        $addName = isset($row['jurisName']) ? $row['jurisName'] : 'Special';
    ...
    public function isAllowedAddressVerification($address)
    {
        ...
        if (\XLite\Core\Config::getInstance()->XC->AvaTax->addressverif) {
            ...
            $result = in_array($assembledAddress['country'], array('US', 'CA'));
        }
    ...
    {
        if (\XLite\Core\Config::getInstance()->XC->AvaTax->debugmode) {
            \XLite\Logger::getInstance()->logCustom('avatax', $message);
        }
    }
