...
class StateTax extends \XLite\Logic\Order\Modifier\ATax
{
     ...
     * Modifier code
     */
    const MODIFIER_CODE = 'AVATAX';

    /**
     * Tax cache TTL
     */
    const AVATAX_CACHE_TTL = '86400';
    ...
    public function canApply()
    {
        ...
        if ($this->order && $this->order->getItems()) {
            $persistentItems = $this->order->getItems()->filter(function ($item) {
    ...
    public function calculate()
    {
        ...
        if ($taxes) {
            if (\XLite\Core\Config::getInstance()->XC->AvaTax->display_as_summary) {
                $taxCost = array_reduce($taxes, function ($cost, $tax) {
