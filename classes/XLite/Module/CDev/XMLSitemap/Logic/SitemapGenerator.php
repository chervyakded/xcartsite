...
class SitemapGenerator extends \XLite\Base\Singleton
{
    ...
    public function getIndex()
    {
        ...
        if ($list) {
            $aliases = \XLite\Model\Repo\CleanURL::getConfigCleanUrlAliases();
            // create clean URLs with index param when an alias is used
            $forceAddGetParams = isset($aliases['sitemap']) && LC_USE_CLEAN_URLS;
            foreach ($list as $path) {
                $name        = basename($path);
                $indexParams = ['index' => substr($name, 11, -4)];
                $cleanURL    = \XLite\Core\Converter::buildURL('sitemap', '', $indexParams, \XLite::getCustomerScript());
                $loc         = \XLite::getInstance()->getShopURL(
                    $cleanURL, null, $forceAddGetParams ? $indexParams : []
                );
                $time = filemtime($path);
                $string .= '<sitemap>'
