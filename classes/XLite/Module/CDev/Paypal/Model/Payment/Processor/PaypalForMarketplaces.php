...
class PaypalForMarketplaces extends \XLite\Module\CDev\Paypal\Model\Payment\Processor\ExpressCheckout
{
    ...
    public function canEnable(\XLite\Model\Payment\Method $method)
    {
        $firstTest = parent::canEnable($method)
            && \XLite\Module\CDev\Paypal\Main::PP_METHOD_PFM === $method->getServiceName()
            && \Includes\Utils\Module\Manager::getRegistry()->isModuleEnabled('XC', 'MultiVendor');

        if ($firstTest) {
            $paypalCommercePlatform = \XLite\Module\CDev\Paypal\Main::getPaymentMethod(\XLite\Module\CDev\Paypal\Main::PP_METHOD_PCP);
        }
