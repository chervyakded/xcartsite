...
class Product extends \XLite\Model\Repo\Product implements \XLite\Base\IDecorator
{
    ...
    const P_HAS_SALE_DISCOUNTS = 'hasSaleDiscounts';
    ...
    {
        if (is_array($value)) {
            /** @var $v \XLite\Module\CDev\Sale\Model\SaleDiscount */
            foreach ($value as $k => $v) {
                ...
                if ($hasSpecificProducts = $v->getSpecificProducts()) {
                    $specificProducts = array_map(function ($saleDiscount) {
                        /** @var $saleDiscount \XLite\Module\CDev\Sale\Model\SaleDiscountProduct */
