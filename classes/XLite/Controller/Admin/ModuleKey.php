    ...
    protected function doActionRegisterKey()
    {
        ...
        if ($result['key'] && $result['action']) {
            ...
            foreach ($result['alert'] as $alert) {
                ...
                } elseif ($alert['message'] === 'activate_license_dialog.result.incompatible.module') {
                    \XLite\Core\TopMessage::addInfo(
                        'The "{{name}}" module version is incompatible with your core version and cannot be enabled.',
                        [
                            'name' => $alertParams[0],
                        ]
                    );
                ...
                } elseif ($alert['message'] === 'module_state_message.license_expired') {
                    \XLite\Core\TopMessage::addError(
                        'The key does not allow getting new features and upgrades.',
                        [
                            'renewalUrl' => $alertParams[0],
                        ]
                    );
                }
            }
