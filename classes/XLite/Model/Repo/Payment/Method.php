...
class Method extends \XLite\Model\Repo\Base\I18n
{
    ...
    public const P_ONLY_PURE_OFFLINE   = 'onlyPureOffline';
    public const P_ONLY_MODULE_OFFLINE = 'onlyModuleOffline';
    ...
    protected function removeUnavailablePaymentMethods($methodsFromMarketplace)
    {
        foreach ($this->findAllMethods() as $method) {
            ...
            if (!$method->isAvailable()
                || ($method->getFromMarketplace() && !in_array($method->getServiceName(), $methodsFromMarketplace))
            ) {
