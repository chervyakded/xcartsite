...
class StructureRegistry
{
    ...
    public static function moveModuleToEnabledRegistry($module)
    {
        $enabledPath = static::getEnabledStructuresPath();
        $enabledRegistry = Operator::loadServiceYAML($enabledPath);
        $enabledRegistry = static::convertRegistryKeys($enabledRegistry);

        $disabledPath = static::getDisabledStructuresPath();
        $disabledRegistry = Operator::loadServiceYAML($disabledPath);
        $disabledRegistry = static::convertRegistryKeys($disabledRegistry);
    ...
    /**
     * Convert registry keys from old format (e.g. CDev\Sale) to new (e.g. CDev-Sale)
     * Needs after 5.3.6.x upgrade
     *
     * @param $registry
     *
     * @return array
     */
    protected static function convertRegistryKeys($registry)
    {
        if (!empty($registry)) {
