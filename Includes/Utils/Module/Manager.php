...
class Manager
{
     ...
     * @throws \MJS\TopSort\CircularDependencyException
     */
    public static function sortSkinModulesByDependency(array $ids)
    {
        $sorter = new StringSort();

        foreach ($ids as $id) {
            $module = static::getRegistry()->getModule($id);
            $dependsOnSkins = array_filter(
                $module->dependsOn,
                static function ($moduleId) {
                    $module = static::getRegistry()->getModule($moduleId);
                    return $module && $module->isSkin();
                }
            );
            $sorter->add($id, $dependsOnSkins);
        }
