 ...
 * @RebuildStep(script = "major-upgrade", weight = "10000")
 */
class MovePacks implements StepInterface
{
    ...
    public function initialize(ScriptState $scriptState, StepState $stepState = null): StepState
    {
        ...
        // move CDev-Core transition to the end for the 5.4->5.5 upgrade
        if (
            isset($transitions['CDev-Core'])
            && version_compare($parentStepState->finishedTransitions['CDev-Core']['version_after'], '5.5', '>=')
        ) {
            $coreTransition = $transitions['CDev-Core'];
            unset($transitions['CDev-Core']);

            $coreTransition['version_after'] = $parentStepState->finishedTransitions['CDev-Core']['version_after'];

            $transitions = array_merge(
                $transitions,
                ['CDev-Core' => $coreTransition]
            );
        }
    ...
    private function processTransition(StepState $state): StepState
    {
        ...
        if ($id === 'CDev-Core' && version_compare($transition['version_after'], '5.5', '>=')) {
            $this->cacheProvider->flushAll();
            ...
            throw HoldException::fromMovePacksStepRedirectTo55($state);
        }
        ...
        try {
            ...
            if ($transition['remove_files'] && $this->resourceChecker->timeRemain() > 5000) {
                foreach ($transition['remove_files'] as $k => $file) {
                    if (
                        $this->filesystem->exists($this->rootDir . $file)
                        && !in_array($file, ['public/upgrade_54_to_55.php', 'public/upgrade_54_to_55/functions.php'], true)
                    ) {
                        $this->backup->addReplaceRecord($file);
