...
class ChangeUnitProcessor
{
    ...
    public function process(array $scenario, array $changeUnits): array
    {
        ...
        $this->logger->debug(
            ...
            [
                ...
                'transitions' => array_map(static function ($transition) {
                    return is_object($transition) ? get_class($transition) : '';
                }, $transitions),
            ]
        );
        ...
        if (!$this->isMajorUpgradeScenario($changeUnits)) {
    ...
    {
        if ($this->isMajorUpgradeScenario($changeUnits)) {
            return $this->sortMajorUpgradeChangeUnits($changeUnits);
        }
     ...
     * @throws ElementNotFoundException
     */
    private function sortMajorUpgradeChangeUnits($changeUnits): array
    {
        $sorter = new StringSort();
        $dependencies = [];

        foreach ($changeUnits as $changeUnit) {
            $moduleId = $changeUnit['id'];
            $module   = $this->installedModulesDataSource->find($moduleId);

            if ($changeUnit['enable'] ?? true) {
                $dependencies[$moduleId] = array_merge($dependencies[$moduleId] ?? [], $module->dependsOn);
            } else {
                $dependencies[$moduleId] = $dependencies[$moduleId] ?? [];

                foreach ($module->dependsOn as $dependencyId) {
                    $dependencies[$dependencyId] = array_merge($dependencies[$dependencyId] ?? [], [$moduleId]);
                }
            }
        }
        ...
        $sortedModuleIds   = $sorter->sort();
        $sortedChangeUnits = [];

        foreach ($sortedModuleIds as $moduleId) {
            $sortedChangeUnits[$moduleId] = $changeUnits[$moduleId];
        }

        return $sortedChangeUnits;
    }

    private function isMajorUpgradeScenario($changeUnits): bool
    {
        return (
            isset($changeUnits['CDev-Core']['version'])
            && version_compare($changeUnits['CDev-Core']['version'], '5.5', '>=')
        );
    }
}
