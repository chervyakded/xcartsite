...
class UpgradeResolver
{
    ...
    public function fillScenario($value, $args, $context, ResolveInfo $info): array
    {
        ...
        $list = $this->getUpgradeListByType($upgradeType);
        ...
        // if upgrade type is major, modules without upgrade must be disabled
        if ($upgradeType === 'major') {
            $disableChangeUnits = array_map(
                static function ($id) {
                    return [
                        'id'     => $id,
                        'enable' => false,
                ...
                $this->getEnabledModuleIdsWithoutMajorUpgrade()
            );

            $changeUnits = array_merge($changeUnits, $disableChangeUnits);
        }
     ...
     * @Resolver()
     */
    public function getEnabledModulesWithoutMajorUpgrade($value, $args, $context, ResolveInfo $info): array
    {
        return array_map(
            function ($moduleId) use ($context) {
                return $this->modulesResolver->getModule($moduleId, $context);
            },
            $this->getEnabledModuleIdsWithoutMajorUpgrade()
        );
    }

    private function getEnabledModuleIdsWithoutMajorUpgrade(): array
    {
        $moduleIds = [];
        $list      = $this->getUpgradeListByType('major');

        foreach ($this->installedModulesDataSource->getAll() as $module) {
            if (
                $module->enabled
                && !isset($list[$module->id])
                && !in_array($module->id, ['CDev-Core', 'XC-Service'])
            ) {
                $moduleIds[] = $module->id;
            }
        }
    ...
    public function getAvailableUpgradeTypes($value, $args, $context, ResolveInfo $info): array
    {
        ...
        // don't show core and major upgrades, if minor, build or self upgrades exist
        if (
            isset($result['minor'])
            || isset($result['build'])
            || isset($result['self'])
        ) {
            unset($result['core'], $result['major']);
        }
     ...
     * @Resolver()
     */
    public function getErrorRequirements($value, $args, Context $context, ResolveInfo $info): array
    {
        $errorRequirements = [];

        if (version_compare(PHP_VERSION, 7.4, '<')) {
            $errorRequirements[] = 'upgrade-details-page.requirements-warning.php.message';
        }

        $extRequirements = ['ctype', 'iconv', 'json', 'pdo', 'sodium'];

        foreach ($extRequirements as $ext) {
            if (!extension_loaded($ext)) {
                $errorRequirements[] = "upgrade-details-page.requirements-warning.{$ext}.message";
            }
        }

        if ($tableNames = $this->dbInfo->getTableNames()) {
            $serviceTables = array_filter(
                $tableNames,
                static function (string $tableName) {
                    return ($tableName === 'sessions')
                        || ($tableName === 'messenger_messages')
                        || (strpos($tableName, 'service_') === 0);
                }
            );

            if ($serviceTables) {
                $errorRequirements[] = sprintf(
                    'We have found the following tables in the store database: %s.
                    These tables were created in the process of upgrading to 5.5.0.x.
                    To continue the upgrade, please delete these tables.',
                    implode(', ', $serviceTables)
                );
            }
        }

        return $errorRequirements;
    }
