...
class LicenseResolver
{
    ...
    {
        $this->removeFreeExpiredLicences();
        ...
        if ($key) {
            ...
            if ($scenario) {
                ...
                if ((int) $key['keyType'] === 2) {
                    ...
                    if (Module::isPreviuosMajorVersion($module->version, $this->coreConfigDataSource->version)) {
                        $result['alert'][] = [
                            'type'    => 'info',
                            'message' => 'activate_license_dialog.result.incompatible.module',
                            'params'  => AlertType::prepareParams([
     ...
     * @Resolver()
     */
    public function remove($value, $args, $context, ResolveInfo $info): array
    {
        if (!($context->mode & Context::ACCESS_MODE_WRITE)) {
            return [];
        }
        ...
        $key = [];
        if (!empty($args['key'])) {
            $key = $this->removeLicense(trim($args['key']));
        }

        if ($key) {
            $result['alert'][] = [
                'type'    => 'success',
                'message' => 'deactivate_license_dialog.result.success',
            ];
        } else {
            $result['alert'][] = [
                'type'    => 'warning',
                'message' => 'deactivate_license_dialog.result.not_found',
            ];
        }
        return $result;
    }
     ...
     * @return bool
     */
    private function removeLicense($licenseKey): bool
    {
        return $this->licenseDataSource->removeByLicenseKey($licenseKey);
    }
    ...
    /**
     * @throws \XCart\Bus\Exception\NotImplemented
     */
    private function removeFreeExpiredLicences()
    {
        // Get all expired modules
        $expiredModuleLicences = array_filter($this->licenseDataSource->getAll(), function ($item) {
            return isset($item['keyType']) && (int) $item['keyType'] !== LicenseDataSource::KEY_TYPE_CORE
                && isset($item['keyData']['expDate']) && $item['keyData']['expDate'] < time();
        }
        ) ?: [];
        // flatten array
        array_walk($expiredModuleLicences, function (&$val, $key) {
            $val = $val['author'] . '-' . $val['name'];
        });

        // get expired paid modules licences to delete according to the new core key
        if (!empty($expiredModuleLicences)) {
            $expiredFreeLicencesToDelete = $this->modulesDataSource->getSlice(
                Flatten::RULE_LAST,
                [
                    'installed'  => true,
                    'payable'    => 'free',// here is the main filter
                    'includeIds' => $expiredModuleLicences,
                ]
            ) ?: [];

            // remove expired (now free) modules licences
            foreach ($expiredFreeLicencesToDelete as $module) {
                $key = implode('-', [$module['author'], $module['name'], '0']);
                $this->licenseDataSource->removeOne($key);
            }
        }
    }
