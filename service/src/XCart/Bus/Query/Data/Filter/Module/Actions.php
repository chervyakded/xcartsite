...
class Actions extends AModifier
{
    ...
    public function current()
    {
        ...
        if ($item['enabled']) {
            ...
            if ($item['dependsOn']) {
                $mustEnabledList = array_filter(
                    $item['dependsOn'],
                    function ($dependModuleId) {
                        /** @var Module $dependedModule */
                        $dependedModule = $this->modulesDataSource->findOne($dependModuleId);

                        return !$dependedModule || $dependedModule->enabled === false;
                    }
                );
                ...
                if ($mustEnabledList) {
                    $messages[] = [
                        ...
                        'params'  => AlertType::prepareParams($this->prepareModulesList($mustEnabledList)),
                    ];
                }
            }
